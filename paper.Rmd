...
Choosing the slice thickness is a compromise between what feature size can be revealed and how many points fall inside a thin slice. As dimensionality $p$ increases, the relative number of points that are inside a slice of fixed thickness $h$ will decrease. The exact relation depends on the distribution of the data points. Here we estimate how it scales with $h$ and $p$ for the example of points uniformly distributed in a hypersphere. The example was selected since it represents a rotation invariant uniform distribution in $p$ space. As we are interested in hollowness this type of distribution is more relevant than a multivariate normal distribution.
...
\end{equation}
where the approximation is valid when $h \ll r$, as is typically expected to be the case. Thus if we want to keep the relative number of points (i.e. $V_{rel}$) approximately constant for slices in different dimensions $p$, we calculate $h$ from a volume parameter as $h = \epsilon^{1/(p-2)}$.
